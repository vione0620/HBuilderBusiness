...
var jpPrinter = {
  createNew: function () {
	...
	jpPrinter.setFontStyle = function (n) { //字体格式
		/* 
		n	效果
		0	取消加粗
		12	倍高宽
	...
	jpPrinter.setFontStyleHan = function (n) { //汉字格式
		/* 
		n	效果
		0	取消加粗
		48	倍高宽
	...
	jpPrinter.setBlackPosizion = function (pL,pH,a,m,nL,nH){ //设置黑标定位偏移量
		data.push(29)
		data.push(40)
		data.push(70)
		data.push(pL)
	...
	jpPrinter.setBlackLeft = function () { //移动到左黑标处
		data.push(12)
	}
	
	jpPrinter.setBlackRight = function () { //移动到右黑标处
		data.push(14)
	}
    ...
    jpPrinter.setPrintL = function () { //打印并回车
      data.push(13)
    }
	...
	jpPrinter.setNextTable = function () { //移动打印位置到下一个水平制表
		data.push(9)
	}
	jpPrinter.setImage = function (m,nL,nH,...dk){
		data.push(27)
		data.push(42)
		data.push(m)
		data.push(nL)
