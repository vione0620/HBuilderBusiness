<template> 
	<view id="months">   
		<view class="chart-group">
			...
			<view class="subtitle">比上一月 <view class="case">{{minusNums}} 
				<view :class="isMinus ? 'green' : 'red'" v-if="!isFair">{{` ${parseFloat(getIncomeData.incomeChange / 100).toFixed(2)} 元`}}</view> 
		...
		<view class="chart-group">
			...
			<view class="subtitle" v-if="!isnoNum">当月收入最高的时间是 <view class="case">{{getAccessData.busiMoney}}</view></view>
		...
		<view class="chart-group">
			...
			<view class="subtitle">当月到店人数比上一月
				<view class="case">{{minusNums}} 
					<view :class="isMinus ? 'green' : 'red'" v-if="!isFair">{{` ${getIncomeData.customerChange} 人`}}</view>
					<view class="black" v-if="isFair">无变化</view> 			
				</view>
			</view>
		...
		<view class="chart-group">
			...
			<view class="subtitle" v-if="!isnoNum">当月到店人数最多的时间是 <view class="case">{{getAccessData.busiDay}}</view></view> 
...
<script>		
	...
	export default {
		...
		created() {			
			...
			this.getIncomeAnalysisMonth(this.chartTime)
			this.getIncomeTrendMonth(this.chartTime)			 			 
		}, 
		computed:{ 
			minusNums:function(){
		...
		methods: {   
			//当月收分析（营收和顾客数）
			getIncomeAnalysisMonth(showDate){
				let vVlue = {"merchNo":this.merchNo,"busiMonth":showDate} //必传
				let sSort = getSortAscii(vVlue) ///排序
				let sSign = hexMD5(sSort + "&key=" + this.loginWhether.md5key).toUpperCase() //转码 
				this.$request.post('getIncomeAnalysisMonth',{
					...vVlue,
					"sign":sSign,
				},{token:true}).then(res=>{ 
									...
									if(res.data.income.length){
										...
										let barperson = []
										 ...
										 for(let i in resdata){
											...
											let time = resdata[i].busiMonth + '月'
			...
			//当月收趋势（营收和访客数）
			getIncomeTrendMonth(showDate){
				let vVlue = {"merchNo":this.merchNo,"busiMonth":showDate} //必传
				let sSort = getSortAscii(vVlue) ///排序
				let sSign = hexMD5(sSort + "&key=" + this.loginWhether.md5key).toUpperCase() //转码 
				this.$request.post('getIncomeTrendMonth',{
					...vVlue,
					"sign":sSign,
				},{token:true}).then(res=>{ 
					if(res.code){
							...
							setTimeout(()=>{ 
								if(res.data == null || res.data == ''){			
									...
									if(res.data){
										...
										this.getAccessData.busiDay = bartime[good_person_index]
									}
								}
