				...
				<view class="bottom-car"> 
					<view class="group-wrap"> 
						<view class="carPop-list" v-for="(food,index) in cartGoods" :key="'4'+index">							
							...
							<view class="nums">
								{{parseFloat(food.merchPrice * food.boxNums /100).toFixed(2) + '元'}}
							</view>
			...
			<view class="left">
				<view class="cars-wrap">
					...
					<view class="nums-wrap">
						<view class="prices">{{parseFloat(totalPrice/100).toFixed(2)}}<text class="txt">元</text></view>							
...
<script>
	...
	import GetBtnItem from './cartbox/get-btn-item.vue' 
	import uniBadge from '../uni/uni-badge.vue'
	export default{
		components:{
			UniPopup,	
			uniBadge,
